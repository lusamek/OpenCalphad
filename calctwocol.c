...
char *strcalc( char *str, double insertvarx, double insertvary )
{ 
      char ptr[PATH_MAX];
      char charo[PATH_MAX];
      ...
      for(i=0; str[i]!='\0' ; i++)
      {
        ...
        if ( str[i] == 'X' )  
        {
           snprintf( charo, PATH_MAX , " ( %.40f ) ", insertvarx );
...
int main( int argc, char *argv[])
{
    ...
    if ( argc == 3 )
     if ( ( strcmp( argv[1] , "-eng" ) ==  0 ) || ( strcmp( argv[1] , "--eng" ) ==  0 ) )
     {
         printf( "Function: %s \n",  argv[ 2 ] );
         printf( "LineCalc: Result Engineering: %.3E \n", te_interp( argv[ 2 ] , 0 ) );
         return 0;
     }
    ...
    if ( argc == 2)
     ...
     double foo; 
     double trange_min  = 0;  
     double trange_max  = 1000;  
     double trange_step = 5;  
     char funcf1[PATH_MAX];
     char funcf2[PATH_MAX];
     char funcf3[PATH_MAX];
     int debug_mode = 0;
     trange_min =  atof( argv[ 1 ]  ); 
     trange_max =  atof( argv[ 2 ]  ); 
     trange_step = atof( argv[ 3 ]  ); 

     for(  foo = trange_min  ; foo <= trange_max ; foo = foo + trange_step )
     {
             printf( " %.40f  \t  %.40f \n", te_interp( strcalc( argv[ 4 ], foo , 0 ), 0) ,    te_interp( strcalc( argv[ 5 ], foo , 0 ), 0)   );
     }
    return 0;
}
